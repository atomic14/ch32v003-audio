...
#define PWM_FREQ        64000      // PWM carrier frequency (Hz)

static const uint32_t SAMPLE_PERIOD_US = (1000000UL / FS_HZ);

Player::Player(TIM_TypeDef *timer, int pwm_channel, GPIO_TypeDef *pwm_gpio_port, int pwm_gpio_pin) {
    this->timer = timer;
...
void Player::play(AudioStream& stream) {
    ...
    while (stream.hasNext())
    {
        ...
        int16_t sample = stream.nextSample();
        uint8_t pwm_value = (uint8_t)((sample >> 8) + 128);
