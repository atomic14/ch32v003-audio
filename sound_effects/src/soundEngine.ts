...
export class OneBitSoundEngine {
  ...
  constructor() {
    if (typeof AudioContext !== 'undefined') {
      ...
      // Try webkit prefixed version for older browsers
  ...
  generateSound(params: SoundParameters): Float32Array {
    ...
    for (let i = 0; i < samples; i++) {
      ...
      // Add vibrato
      const vibratoAmount =
        Math.sin((2 * Math.PI * params.vibratoSpeed * t * params.duration) / 1000) * params.vibrato;
  ...
  exportToCArray(params: SoundParameters, variableName: string = 'sound_effect'): string {
    ...
    lines.push(
      `// 1-bit sound effect - ${params.startFreq}Hz to ${params.endFreq}Hz, ${params.duration}ms`
    );
