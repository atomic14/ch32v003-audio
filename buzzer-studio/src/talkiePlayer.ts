...
export function initTalkiePlayer(container: HTMLElement): void {
    ...
    <div class="talkie-player">
      ...
      <div class="section">
        ...
        <div class="sample-buttons">
          ${Object.keys(SAMPLE_PHRASES).map(name =>
            `<button class="sample-btn" data-phrase="${name}">${name}</button>`
          ).join('')}
        </div>
      </div>
  ...
  function generateWaveformPreview(): void {
    ...
    setTimeout(() => {
      try {
        const device = parseInt(deviceSelect.value) as TalkieDevice;
  ...
  function debounceWaveformUpdate(): void {
    if (waveformDebounceTimer) {
      clearTimeout(waveformDebounceTimer);
    }
