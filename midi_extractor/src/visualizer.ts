...
export class MidiVisualizer {
  ...
  private scrollToPlayhead() {
    ...
    const padding = this.options.compact ? 10 : 40;
    const leftPadding = this.options.showLabels ? padding : 10;
    const playheadX = leftPadding + (this.currentTime / this.duration) * (this.canvas.width - leftPadding - padding);
  ...
  draw() {
    ...
    if (this.currentTime > 0) {
      const playheadX = leftPadding + (this.currentTime / this.duration) * (width - leftPadding - padding);
